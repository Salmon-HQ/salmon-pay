...
export async function GET(request: Request) {
  ...
  return Response.json({
    data: (await listRecords<WebhookEndpoint>('webhooks')).map((item) => ({
      id: item.id,
      url: item.url,
      events: item.events,
      active: item.active,
...
export async function POST(request: Request) {
  ...
  let url: string;
  try {
    url = await assertWebhookDestinationSafe(input.url);
  } catch (error) {
    return Response.json(
      {
        error: {
  ...
  const id = `wh_${crypto.randomUUID().replaceAll('-', '').slice(0, 12)}`,
    secret = `whsec_${crypto.randomUUID().replaceAll('-', '')}`,
    endpoint: WebhookEndpoint = {
      id,
      url,
      events: input.events ?? ['payment.paid'],
      active: true,
      secretCiphertext: await encryptSecret(secret),
      createdAt: new Date().toISOString(),
    };
  await putRecord('webhooks', id, endpoint);
  return Response.json(
    {
      data: {
        id,
